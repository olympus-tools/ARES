...
build-executable: setup-venv
	...
	@"$(VENV_BIN)/pyinstaller" --onefile --name ares --paths . --paths submodules/dcmi --add-data "ares/plugins/simunit.py$(PATHSEP)ares/plugins" --add-data "ares/plugins/merge.py$(PATHSEP)ares/plugins" --hidden-import "ares.pydantic_models.datadictionary_model" --hidden-import "dcmi" --hidden-import "dcmi.dcmi" ares/__main__.py
	@echo ""
