...
def test_workflow_output_path_inserts_timestamp_before_compound_suffix(
    tmp_path: Path,
):
    """Tests that workflow timestamps precede the complete workflow suffix."""
    workflow = Workflow.__new__(Workflow)
    workflow._file_path = tmp_path / "simunit_vstack.wf.json"

    output_path = workflow._eval_output_path(
        dir_path=tmp_path / "output", output_format="json"
    )

    assert re.fullmatch(r"simunit_vstack__\d{14}\.wf\.json", output_path.name)
