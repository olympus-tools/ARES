...
class SimUnit:
    def __init__(
        ...
        :param file_path: The path to the shared library file (e.g., .so, .dll, .dylib).
        :param dd_path: The path to the Data Dictionary JSON file.
        :param logfile: The logfile object for logging.
        :return: None. The constructor initializes all necessary instance variables.
        """
    ...
    def _load_dd(self, dd_path: str) -> dict | None:
        ...
        :param dd_path: The path to the Data Dictionary JSON file.
        :return: The loaded Data Dictionary as a dictionary, or `None` if an error occurs.
        """
        try:
    ...
    def _dd_validation(self, dd: dict, dd_path: str):
        ...
        predefined JSON schema. The function logs success or detailed error messages.

        :param dd: The Data Dictionary dictionary to be validated.
        :param dd_path: The file path of the Data Dictionary for logging purposes.
        """
        try:
    ...
    def _load_library(self, file_path: str) -> ctypes.CDLL | None:
        """
        Loads a shared library file using `ctypes`. This makes the symbols (variables
        and functions) from the C library accessible in Python.

        :param file_path: The path to the shared library file.
        :return: The loaded `ctypes.CDLL` object, or `None` if the library cannot be loaded.
        """
        try:
    ...
    def _setup_c_interface(self) -> dict | None:
        """
        Maps global variables defined in the Data Dictionary to their corresponding
        ctypes objects in the loaded shared library. This process involves translating
        the datatype and size information from the DD into `ctypes` types.

        :return: A dictionary where keys are the variable names from the DD and values
    ...
    def _setup_sim_function(self) -> Union[ctypes.CFUNCTYPE, None]:
        ...
        :return: The `ctypes` function object for `ares_simunit`, or `None` if the
    ...
    def run_simulation(self, data: dict, parameter: dict) -> dict | None:
        ...
        :param data: A dictionary containing all input signals, including a 'timestamp' array.
                    Example: `{"timestamp": [t0, t1], "signal_A": [v0, v1]}`.
        :param parameter: A dictionary containing simulation parameters (currently unused but included
                        for future expansion).
        :return: A dictionary of NumPy arrays containing the output signals for all time steps,
                or `None` if an error occurs during the simulation.
        """
    ...
    def _map_sim_input(self, input_data: dict, time_steps: int) -> dict | None:
        ...
        :param input_data: The input data dictionary (e.g., from a data source).
        :param time_steps: The total number of simulation steps.
        :return: A dictionary of mapped input values, ready for the `_write_dll_interface` method,
                or `None` if a mapping error occurs.
        """
        try:
            mapped_input = {}
    ...
    def _map_sim_input_static(
        ...
        :param time_steps: The total number of simulation steps.
        :param datatype: The target datatype for the NumPy array (e.g., 'float', 'int').
        :param size: The size of the simulation variable (e.g., `[1]` for a scalar, `[10]` for an array).
        :param value: The constant value to assign to the variable.
        :return: A NumPy array containing the constant value for all time steps, or `None` if an error occurs.
        """
        try:
            if len(size) == 1:
    ...
    def _write_dll_interface(self, input: dict, time_step_idx: int):
        ...
        global C variables in the shared library. It only writes to variables marked
        as `'in'` or `'inout'` in the Data Dictionary.

        :param input: A dictionary with variable names and their values for the current time step.
        :param time_step_idx: The index of the current time step.
        :return: None. The function directly modifies the memory of the C library. Errors are logged.
        """
        for dll_interface_key in self.dll_interface.keys():
            try:
    ...
    def _read_dll_interface(self) -> dict | None:
        ...
        :return: A dictionary where keys are the variable names and values are their current
                values read from the C library for the current time step.
        """
        current_values = {}
        for dll_interface_key in self.dll_interface.keys():
            try:
