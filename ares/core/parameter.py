...
class Parameter:
    ...
    def __init__(self, file_path: str, logfile: Logfile):
        ...
        if input_format == ".json":
            self.parameter: ParameterModel = ParamJSONinterface.load(
                file_path=self._file_path, logfile=self._logfile
            )
        elif input_format == ".dcm":
            self.parameter: ParameterModel = ParamDCMinterface.load(
                file_path=self._file_path, logfile=self._logfile
            )
        else:
            self._logfile.write(
    ...
    @typechecked
    def write_out(
        self, dir_path: str, output_format: str, meta_data: dict
    ) -> str | None:
        """
        ...
        Returns:
            str | None: Full path to the written file, or None if writing fails.
        """
    ...
    def _eval_output_path(self, dir_path: str, output_format: str) -> str | None:
        ...
        The format is `*_YYYYMMDDHHMMSS*` before the file extension.
